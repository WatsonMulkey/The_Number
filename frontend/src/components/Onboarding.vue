<template>
  <v-card class="onboarding-card mx-auto" elevation="4" max-width="900" color="#E9F5DB">
    ...
    <div class="progress-dots py-3" v-if="currentStep > 0" role="progressbar" :aria-valuenow="currentStep" aria-valuemin="1" aria-valuemax="5" :aria-label="`Step ${currentStep} of 5`">
      <span
        v-for="step in 5"
        :key="step"
        :class="['progress-dot', { 'active': step <= currentStep, 'current': step === currentStep }]"
        :aria-hidden="true"
      />
    </div>
    ...
    <v-card-text>
      ...
      <div v-else-if="currentStep === 1">
        <h2 class="text-h5 mb-3 text-center">Choose Your Budgeting Style</h2>
        <p class="text-body-1 text-center onboarding-secondary-text mb-4">Which situation describes you best?</p>
          ...
          >
            <v-radio value="paycheck">
              <template v-slot:label>
                <div class="ml-4">
                  <div class="text-subtitle-1 font-weight-bold mb-2" style="color: var(--color-text-primary);">
                    <v-icon color="success" class="mr-2">mdi-cash-check</v-icon>
                    Paycheck Mode
                  </div>
                  <div class="text-body-2" style="color: var(--color-soft-charcoal);">
                    I have regular income (weekly, bi-weekly, or monthly paychecks)
                  </div>
                  <div class="text-caption mt-1" style="color: var(--color-text-secondary);">
          ...
          >
            <v-radio value="fixed_pool">
              <template v-slot:label>
                <div class="ml-4">
                  <div class="text-subtitle-1 font-weight-bold mb-2" style="color: var(--color-text-primary);">
                    <v-icon color="success" class="mr-2">mdi-piggy-bank</v-icon>
                    Fixed Pool Mode
                  </div>
                  <div class="text-body-2" style="color: var(--color-soft-charcoal);">
      ...
      <div v-else-if="currentStep === 2">
        ...
        <v-form ref="detailsForm" class="max-w-500 mx-auto" @submit.prevent="nextStep">
          ...
          <div v-if="budgetMode === 'paycheck'">
            <p class="text-subtitle-1 font-weight-medium mb-3">How often do you get paid?</p>
          ...
          <div v-if="budgetMode === 'fixed_pool'">
            ...
            <p class="text-subtitle-1 font-weight-medium mb-2">How would you like to set up your budget?</p>
            ...
            <v-radio-group v-model="fixedPoolMode">
              ...
              >
                <v-radio value="target_date">
                  <template v-slot:label>
                    <div class="ml-3">
                      <div class="text-subtitle-1 font-weight-bold mb-1" style="color: var(--color-text-primary);">
                        <v-icon size="small" color="success" class="mr-1">mdi-calendar-clock</v-icon>
              ...
              >
                <v-radio value="daily_limit">
                  <template v-slot:label>
                    <div class="ml-3">
                      <div class="text-subtitle-1 font-weight-bold mb-1" style="color: var(--color-text-primary);">
                        <v-icon size="small" color="success" class="mr-1">mdi-cash-limit</v-icon>
      ...
      <div v-else-if="currentStep === 3">
        <h2 class="text-h5 mb-3 text-center">Add Your Monthly Expenses</h2>
        <p class="text-body-1 text-center mb-4" style="color: var(--color-soft-charcoal);">
          Add expenses you MUST pay each month (rent, utilities, bills)
        </p>
      ...
      <div v-else-if="currentStep === 4">
        <div class="text-center">
          ...
          <h2 class="text-h5 mb-4">Here's Your Number!</h2>
