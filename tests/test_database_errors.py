...
class TestExpenseErrorCases:
    ...
    def test_add_expense_negative_amount_rejected(self):
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix='.db') as f:
            db = BudgetDatabase(f.name, encryption_key=TEST_KEY)
    ...
    def test_update_expense_nonexistent_id_rejected(self):
        """Test that updating a non-existent expense raises an error.

        This prevents silent failures where the user thinks they updated
        an expense but nothing actually changed.
        """
        with tempfile.NamedTemporaryFile(delete=False, suffix='.db') as f:
            db = BudgetDatabase(f.name, encryption_key=TEST_KEY)
